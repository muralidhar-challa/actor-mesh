#!/usr/bin/env python3
"""
Export spaCy /tmp/ner_floret_model/model-last full NER pipeline to ONNX.
...
def build_floret_embed(g, col_slice, nV, E):
    """
    Floret-style embedding: single table, double-hash.
    col_slice: (seq_len, 2) uint64 — two FNV-1a hashes per token
    Hash1 -> row index (mod nV), Hash2 -> sign flip.
    Single Gather + optional Mul by {-1, 1}.
    Simpler graph than the 4-table Bloom approach.
    """
...
def load_model():
    import spacy
    print("Loading /tmp/ner_floret_model/model-last ...")
    nlp = spacy.load("/tmp/ner_floret_model/model-last")
...
def export(output_path):
    ...
    for col_idx, chain in enumerate(concat_he._layers):
        ...
        embeds.append(build_floret_embed(g, col4, nV, E))
    ...
    _nlp    = _spacy.load("/tmp/ner_floret_model/model-last")
    ...
    _nlp2  = _spacy.load("/tmp/ner_floret_model/model-last")
