...
int main(void) {
    printf("Tool Call Debug Test\n");
    free_ports(); cleanup();
    system("rm -rf /tmp/tc-test; mkdir -p /tmp/tc-test/{reg,db,ag}");
    ...
    char *renv[] = {
        ...
        "ACTOR_LMDB_PATH=/tmp/tc-test/reg", blib, NULL};
    ...
    char *senv[] = {
        ...
        "ACTOR_LMDB_PATH=/tmp/tc-test/db",
        blib, mcp_srv, "MCP_TOOL=read_query", "MCP_ARG=sql", emp_db, NULL};
    pid_t spid = sp(sargs, senv); ms(600);

    /* ── Test: direct SQL query via MCP ── */
    TEST("MCP bridge: direct SQL call");
    {
        /* mpack: {type:"sql_query", sql:"SELECT COUNT(*) FROM employees"} */
        uint8_t qm[] = {0x82,0xa4,'t','y','p','e',0xa9,'s','q','l','_','q','u','e','r','y',
            0xa3,'s','q','l',0xa7,'S','E','L','E','C','T',' ','1'};
        send_msg("sql_query", qm, sizeof(qm));
        uint8_t buf[4096];
        ssize_t n = wait_msg("sql_result", buf, sizeof(buf)-1, 5000);
        CHECK(n > 0, "MCP bridge did not respond to SQL query");
        if (n > 0) { buf[n]=0; printf("  SQL result: %.*s\n", (int)n, buf); }
    }

    /* ── Test: agent processes user_message end-to-end ── */
    TEST("agent: user_message → agent_response (60s)");
    {
        /* Start agent fresh */
        ...
        char *aenv[] = {
            ...
            "ACTOR_TOPIC=user_message,sql_result,_tool_list",
            "ACTOR_RESULT_TOPIC=agent_response",
            "ACTOR_HANDLER=examples/employee-mesh/handlers/agents/llm-agent",
            "ACTOR_LMDB_PATH=/tmp/tc-test/ag",
            "LLM_BASE_URL=http://localhost:11434", model_env, NULL};
        pid_t apid = sp(aargs, aenv); ms(2000);

        /* Simple chat test */
        uint8_t qm[] = {0x82,0xa4,'t','y','p','e',0xad,'u','s','e','r','_','m','e','s','s','a','g','e',
            0xa5,'q','u','e','r','y',0xa2,'h','i'};
        send_msg("user_message", qm, sizeof(qm));
        uint8_t buf[4096];
        ssize_t n = wait_msg("agent_response", buf, sizeof(buf)-1, 65000);
        CHECK(n > 0, "agent did not respond to chat");
        if (n > 0) { buf[n]=0; printf("  agent: %.*s\n", n<150?(int)n:150, buf); }
        kill(apid, SIGKILL); waitpid(apid, NULL, 0);
    }

    printf("\n%s (%d failures)\n", failures?"FAIL":"ALL PASSED", failures);
    kill(ppid,SIGKILL); kill(rpid,SIGKILL); kill(spid,SIGKILL);
    waitpid(ppid,NULL,0); waitpid(rpid,NULL,0); waitpid(spid,NULL,0);
    cleanup();
    return failures;
}

    /* ── Test: full ReAct loop with tool ── */
    TEST("agent: full ReAct with SQL tool (120s)");
    {
        /* Announce tool */
        uint8_t announce[] = {
        ...
        /* Send query that needs SQL */
        uint8_t qm[] = {0x82,0xa4,'t','y','p','e',0xad,'u','s','e','r','_','m','e','s','s','a','g','e',
            0xa5,'q','u','e','r','y',0xb1,'h','o','w',' ','m','a','n','y',' ','e','m','p','l','o','y','e','e','s',' ','a','r','e',' ','t','h','e','r','e','?'};
        send_msg("user_message", qm, sizeof(qm));

        uint8_t buf[8192];
        ssize_t n = wait_msg("agent_response", buf, sizeof(buf)-1, 125000);
        CHECK(n > 0, "agent did not complete ReAct loop");
        if (n > 0) { buf[n]=0; printf("  final answer: %.*s\n", n<200?(int)n:200, buf); }
    }
